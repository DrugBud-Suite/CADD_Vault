...
- **[MzDock](https://github.com/Muzatheking12/MzDOCK)** : An Automated GUI based pipeline for Molecular Docking
- **[RxDock](https://github.com/rxdock/rxdock)**: a fast, versatile, and open-source program for docking ligands to proteins and nucleic acids (standalone).
- **[FitDock](http://cao.labshare.cn/fitdock/php/register.php)**: protein–ligand docking by template fitting (Linux or Windows, need registration) (standalone).
- **[restretto](https://github.com/akiyamalab/restretto)**: Effective Protein–Ligand Docking Strategy via Fragment Reuse and a Proof-of-Concept Implementation (standalone).
- **[ProBiS-Dock](http://insilab.org/probisdock)**: A Hybrid Multitemplate Homology Flexible Docking Algorithm Enabled by Protein Binding Site Comparison (standalone).
- **[BR-NiB](https://github.com/jvlehtonen/brutenib)**: Brute Force Negative Image-Based Optimization. A docking rescoring method that relyes on shape/ electrostatic potential similarity between the docking poses of ligands and the cavity-based negative images (standalone).
...
- **[Uni-Dock](https://github.com/dptech-corp/Uni-Dock)**: GPU-Accelerated Docking Enables Ultralarge Virtual Screening (standalone).
- **[DeepBindBC](http://cbblab.siat.ac.cn/DeepBindBC/index.php)**: A practical deep learning method for identifying native-like protein-ligand complexes in virtual screening (online).
