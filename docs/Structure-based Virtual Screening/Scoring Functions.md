
## **Carbohydrates**
- **VinaCarb**: Carbohydrate specific scoring function
	- [Publication](https://doi.org/10.1021/acs.jctc.5b00834) : Citations: 89.0
	- [Link](https://pubs.acs.org/doi/10.1021/acs.jctc.5b00834)

## **ElectroStatic Potential-based**
- **ECScore**: Modified Electrostatic Complementary Score Function
	- [Publication](https://doi.org/10.1021/acs.jcim.2c00616) : Citations: 3.0

## **Electron Density**
- **ExptGMS**: Experimental Electron Density based Grid Matching Score
	- [Publication](https://doi.org/10.1038%2Fs42004-023-00984-5) : Citations: 1.0
	- [Webserver](https://exptgms.stonewise.cn/#/create)

## **Empirical**
- **AA-Score**: An empirical protein-ligand scoring function focusing on amino acid-specific interaction components for improved virtual screening and lead optimization.
	- [Code](https://github.com/xundrug/AA-Score-Tool) : Last updated in March 2023, 15 months ago
	- [Publication](https://doi.org/10.1021/acs.jcim.1c01537) : Citations: 12.0
- **LinF9**: Presents Lin_F9, a linear empirical scoring function for protein-ligand docking, available within a fork of the Smina docking suite.
	- [Code](https://github.com/cyangNYU/Lin_F9_test) : Last updated in October 2021, 32 months ago
	- [Publication](https://doi.org/10.1021/acs.jcim.1c00737) : Citations: 14.0
- **Vinardo**: Vinardo is a scoring function based on Autodock Vina that improves scoring, docking, and virtual screening capabilities. It was trained through a combination of scoring, minimization, and re-docking on curated datasets for optimum docking performance. Vinardo is available within Smina, a fork of Vina.
	- [Code](http://smina.sf.net/) : Last updated in nan, nan
	- [Publication](https://doi.org/10.1371/journal.pone.0155183) : Citations: 215.0
	- [Link](https://journals.plos.org/plosone/article?id=10.1371/journal.pone.0155183)

## **Hybrid**
- **SQM-ML**: Hybrid Semiempirical Quantum Chemical - Machine Learning (SQM-ML) scoring function for protein-ligand interactions
	- [Code](https://github.com/tevang/sqm-ml) : Last updated in November 2022, 19 months ago
	- [Publication](https://doi.org/10.26434/chemrxiv-2022-68n6h) : Citations: 0.0

## **Hydration**
- **HydraMapSF**: Contains content related to HydraMap-based features generation, but specific details about its functionality or applications were not provided.
	- [Code](https://github.com/xiaoyangqu/HydraMapSF) : Last updated in March 2023, 15 months ago
- **WatSite3**: 
	- [Link](https://www.nature.com/articles/s42004-020-0261-x#code-availability)

## **Knowledge-based**
- **Convex-PL**: Convex-PL is a knowledge-based scoring function for protein-ligand interactions that augments Convex-PL with conformational flexibility for better performance in affinity prediction and virtual screening tasks.
	- [Publication](https://doi.org/10.1007/s10822-017-0068-8) : Citations: 24.0
	- [Link](https://team.inria.fr/nano-d/software/convex-pl/)
- **DLIGAND2**: DLIGAND2 is a knowledge-based method for predicting protein-ligand binding affinity, utilizing a distance-scaled, finite, ideal-gas reference state.
	- [Code](https://github.com/yuedongyang/DLIGAND2) : Last updated in February 2019, 64 months ago
	- [Publication](https://doi.org/10.1186/s13321-019-0373-4) : Citations: 34.0
- **ITScoreAff**: iterative knowledge-based scoring function for protein-ligand interactions by considering binding affinity information
	- [Publication](https://doi.org/10.1021/acs.jpcb.3c04421) : Citations: 1.0
	- [Link](http://huanglab.phys.hust.edu.cn/ITScoreAff/)
- **KORP-PL**: KORP-PL is a novel coarse-grained knowledge-based scoring function for protein-ligand interactions, focusing on the relative orientation of a ligand molecule to a protein residue.
	- [Publication](https://doi.org/10.1093/bioinformatics/btaa748) : Citations: 16.0
	- [Link](https://team.inria.fr/nano-d/software/korp-pl/)

## **Machine-learning scoring functions**
- **AEScore**: Learning protein-ligand binding affinity using atomic environment vectors.
	- [Code](https://github.com/bigginlab/aescore) : Last updated in March 2022, 26 months ago
	- [Publication](https://doi.org/10.1039/C6SC05720A) : Citations: 1144.0
- **AKScore**: This model uses an ensemble of multiple independently trained 3-D convolutional neural networks to predict protein-ligand complex binding affinity, showing high correlation with experimental data.
	- [Publication](https://doi.org/10.3390/ijms21228424) : Citations: 55.0
	- [Link](https://pubmed.ncbi.nlm.nih.gov/33182567/)
- **APBScore**: Atom Pair Based scoring function.
	- [Code](https://github.com/BaoJingxiao/APBScore) : Last updated in August 2020, 46 months ago
	- [Publication](https://doi.org/10.1021/acs.jcim.0c00474) : Citations: 11.0
- **CAPLA**: Improved prediction of protein-ligand binding affinity by a deep learning approach based on a cross-attention mechanism.
	- [Code](https://github.com/lennylv/CAPLA) : Last updated in November 2022, 19 months ago
	- [Publication](https://doi.org/10.1093/bioinformatics/btad049) : Citations: 17.0
- **Censible**: Predicts small-molecule binding affinities using deep-learning context explanation networks for interpretable output.
	- [Code](https://github.com/durrantlab/censible/) : Last updated in May 2024, 1 months ago
- **CNN**: Files for repeating a study on convolutional neural networks and atomic contact features for binding affinity prediction.
	- [Code](https://github.com/miladrayka/convolutional_neural_networks) : Last updated in March 2023, 15 months ago
- **ConBAP**: Protein-Ligand Binding Affinity Prediction with Multimodal Contrastive Learning
	- [Code](https://github.com/DingLuoXMU/ConBAP) : Last updated in April 2024, 1 months ago
	- [Publication](https://doi.org/10.5281/zenodo.10532672).) : Citations: 0.0
- **Cyscore**: An empirical scoring function for accurate protein-ligand binding affinty prediction (linux command line) (standalone).
	- [Code](http://clab.labshare.cn/software/) : Last updated in nan, nan
	- [Publication](https://doi.org/10.1186/1471-2105-15-291) : Citations: 70.0
	- [Link](http://clab.labshare.cn/software/cyscore.html)
- **DAAP**: 
	- [Code](https://gitlab.com/mahnewton/daap) : Last updated in nan, nan
	- [Publication](https://doi.org/10.1186/s13321-024-00844-x) : Citations: 0.0
- **DeepAffinity**: Protein-compound affinity prediction through unified RNN-CNN.
	- [Code](https://github.com/Shen-Lab/DeepAffinity) : Last updated in June 2021, 36 months ago
	- [Publication](https://doi.org/10.1093/bioinformatics/btz111) : Citations: 297.0
- **DeepBindGCN**: Predicts protein ligand binding affinity non-dependent on docking conformation.
	- [Code](https://github.com/haiping1010/DeepBindGCN) : Last updated in April 2023, 14 months ago
	- [Publication](https://doi.org/10.3390/molecules28124691) : Citations: 1.0
- **DeepBindRG**: A deep learning based method for estimating effective protein-ligand affinity.
	- [Code](https://github.com/haiping1010/DeepBindRG) : Last updated in August 2019, 58 months ago
	- [Publication](https://doi.org/10.7717%2Fpeerj.7362) : Citations: 66.0
- **DeepGLSTM**: A model predicting binding affinity values between FDA-approved drugs and viral proteins of SARS-CoV-2.
	- [Code](https://github.com/MLlab4CS/DeepGLSTM) : Last updated in November 2022, 19 months ago
	- [Publication](https://doi.org/10.1137/1.9781611977172.82) : Citations: 15.0
- **ECIF**: Extended Connectivity Interaction Features for molecular analysis.
	- [Code](https://github.com/DIFACQUIM/ECIF) : Last updated in September 2021, 33 months ago
	- [Publication](https://doi.org/10.1093/bioinformatics/btaa982) : Citations: 55.0
- **EGGNet**: Source code for EGGNet, a framework for protein complex pose scoring.
	- [Code](https://github.com/aws-samples/eggnet-equivariant-graph-of-graph-neural-network) : Last updated in April 2023, 13 months ago
	- [Publication](https://doi.org/10.1101/2023.03.22.533800v1) : Citations: 0.0
- **ENS-Score**: 
	- [Code](https://github.com/miladrayka/ENS_Score) : Last updated in February 2024, 3 months ago
	- [Publication](https://doi.org/10.1002/minf.202300292) : Citations: 1.0
- **ENS-Score GUI**: 
	- [Code](https://github.com/miladrayka/ENS_SCORE_GUI) : Last updated in February 2024, 3 months ago
- **EquiScore**: A scoring function for virtual screening and eval with interpretable output.
	- [Code](https://github.com/CAODH/EquiScore) : Last updated in January 2024, 5 months ago
	- [Publication](https://doi.org/10.1101/2023.06.18.545464v2) : Citations: 0.0
- **ET-Score**: A scoring function based on Extra Trees algorithm for predicting ligand-protein binding affinity.
	- [Code](https://github.com/miladrayka/ET_Score) : Last updated in July 2023, 11 months ago
	- [Publication](https://doi.org/10.1002/minf.202060084) : Citations: 10.0
- **FABind**: A fast and accurate protein-ligand binding model for NeurIPS 2023.
	- [Code](https://github.com/QizhiPei/FABind) : Last updated in June 2024, 0 months ago
	- [Publication](https://arxiv.org/abs/2310.06763) : Citations: N/A
- **FAST**: Fusion models for Atomic and molecular STructures aiming at predicting accurate protein-ligand binding affinity.
	- [Code](https://github.com/llnl/fast) : Last updated in July 2023, 11 months ago
	- [Publication](https://doi.org/10.1021/acs.jcim.0c01306) : Citations: 142.0
- **FGNN**: Predicts ligand binding affinity with graph neural networks and 3D structure-based complex graph.
	- [Code](https://github.com/LinaDongXMU/FGNN) : Last updated in September 2023, 9 months ago
	- [Publication](https://doi.org/10.1039/D3CP03651K) : Citations: 2.0
- **FusionDTA**: A framework focusing on protein-ligand binding affinity prediction with deep learning techniques.
	- [Code](https://github.com/yuanweining/FusionDTA) : Last updated in February 2024, 3 months ago
	- [Publication](https://doi.org/10.1093/bib/bbab506) : Citations: 45.0
- **GBScore**: A scoring function based on Gradient Boosting Trees algorithm for predicting ligand-protein binding affinity.
	- [Code](https://github.com/miladrayka/GB_Score) : Last updated in July 2023, 11 months ago
	- [Publication](https://doi.org/10.1002/minf.202200135) : Citations: 2.0
- **GenScore**: A scoring framework for predicting protein-ligand binding affinity.
	- [Code](https://github.com/sc8668/GenScore) : Last updated in January 2023, 16 months ago
	- [Publication](https://doi.org/10.1039%2Fd3sc02044d) : Citations: 10.0
- **GGL-ETA-Score**: This code computes features for both the SYBYL-GGL and ECIF-GGL models.
	- [Code](https://github.com/NguyenLabUKY/GGL-ETA-Score) : Last updated in January 2023, 16 months ago
- **GIGN**: Contains GNN-based models for protein-ligand binding affinity prediction.
	- [Code](https://github.com/guaguabujianle/GIGN) : Last updated in August 2023, 10 months ago
- **GraphBAR**: deep-learning-based prediction model based on a graph convolutional neural network.
	- [Code](https://github.com/jtson82/graphbar) : Last updated in January 2022, 29 months ago
- **graphLambda**: A deep learning model to score protein-ligand binding affinity using PyTorch and PyTorch Geometric, focusing on the application of graphLambda model for training and result replication.
	- [Code](https://github.com/i-Molecule/graphLambda) : Last updated in February 2024, 4 months ago
	- [Publication](https://doi.org/10.5281/zenodo.10620447).) : Citations: 0.0
- **GraphscoreDTA**: A graph neural network for protein-ligand binding affinity prediction.
	- [Code](https://github.com/CSUBioGroup/GraphscoreDTA) : Last updated in May 2024, 0 months ago
	- [Publication](https://doi.org/10.1093/bioinformatics/btad340) : Citations: 14.0
- **HAC-Net**: A Hybrid Attention-Based Convolutional Neural Network for Highly Accurate Protein-Ligand Binding Affinity Prediction.
	- [Code](https://github.com/gregory-kyro/HAC-Net/) : Last updated in May 2024, 0 months ago
	- [Publication](https://doi.org/10.1021/acs.jcim.3c00251) : Citations: 18.0
- **HaPPy**: Utilizes multi-perspective graphs for protein-ligand binding affinity prediction.
	- [Code](https://github.com/Jthy-af/HaPPy) : Last updated in June 2023, 12 months ago
	- [Publication](https://doi.org/10.1007/s12539-023-00582-y) : Citations: 0.0
- **HGScore**: A Heterogeneous Graph Convolutional Neural Network to score a protein-ligand complex.
	- [Code](https://github.com/KevinCrp/HGScore) : Last updated in November 2023, 6 months ago
	- [Publication](https://doi.org/10.37349/eds.2023.00010) : Citations: 0.0
- **KDeep**: protein-ligand affinity predictor based on DCNNs (Deep Convolutional Neural Networks).
	- [Publication](https://doi.org/10.1021/acs.jcim.7b00650) : Citations: 584.0
	- [Webserver](https://playmolecule.org/Kdeep/)
- **KIDA**: Process for extracting protein pockets and generating training data for KIDA.
	- [Code](https://github.com/RuiqiangLu/KIDA) : Last updated in April 2023, 14 months ago
	- [Publication](https://doi.org/10.1093/bib/bbad145) : Citations: 4.0
- **Lee2023a**: Meta-modeling of ligand-protein binding affinity.
	- [Code](https://github.com/Lee1701/Lee2023a) : Last updated in May 2024, 0 months ago
	- [Publication](https://arxiv.org/abs/2310.03946) : Citations: N/A
- **MBI-Score**: Utilizes graph representation for protein-ligand interfacial atoms in native complexes for scoring.
	- [Link](https://khashanlab.org/mbi-score/)
- **MGraphDTA**: Implements a high-efficiency concordance index for performance evaluation of drug-target affinity prediction.
	- [Code](https://github.com/guaguabujianle/MGraphDTA) : Last updated in April 2024, 2 months ago
	- [Publication](https://doi.org/10.1039/D1SC05180F) : Citations: 107.0
- **ML-PLIC**: 
	- [Publication](https://doi.org/10.1093/bib/bbad295) : Citations: 0.0
	- [Webserver](http://cadd.zju.edu.cn/plic/)
- **MSECIFv2**: Improved version of extended connectivity interaction features (ECIF) by enabling to take the atomic distance into account.
	- [Code](https://github.com/koji11235/MSECIFv2) : Last updated in October 2023, 7 months ago
	- [Publication](https://doi.org/10.1093/bioadv/vbad155) : Citations: 0.0
- **OctSurf**: Efficient hierarchical voxel-based molecular surface representation for protein-ligand affinity prediction (scoring) (standalone).
	- [Code](https://github.uconn.edu/mldrugdiscovery/OctSurf) : Last updated in nan, nan
	- [Publication](https://doi.org/10.1016/j.jmgm.2021.107865) : Citations: 19.0
	- [Link](https://github.uconn.edu/mldrugdiscovery/OctSurf)
- **OnionNet**: A multiple-layer inter-molecular contact based deep neural network for protein-ligand binding affinity prediction.
	- [Code](https://github.com/zhenglz/onionnet/) : Last updated in November 2020, 43 months ago
	- [Publication](https://doi.org/10.1021/acsomega.9b01997) : Citations: 185.0
- **OnionNet-2**: OnionNet-2 is constructed based on convolutional neural network (CNN) to predict the protein-ligand binding affinity.
	- [Code](https://github.com/zchwang/OnionNet-2/) : Last updated in March 2024, 3 months ago
	- [Publication](https://doi.org/10.3389/fchem.2021.753002/full) : Citations: 0.0
- **OnionNet-SFCT**: Incorporates a scoring function correction term for improved docking and screening accuracies.
	- [Code](https://github.com/zhenglz/OnionNet-SFCT) : Last updated in May 2024, 1 months ago
	- [Publication](https://doi.org/10.1093/bib/bbac051) : Citations: 48.0
- **PAMNet**: Official implementation of PAMNet in the paper "A universal framework for accurate and efficient geometric deep learning of molecular systems" accepted by Nature Scientific Reports. PAMNet improves upon MXMNet and achieves high performance in tasks like small molecule property prediction, RNA 3D structure prediction, and protein-ligand binding affinity prediction with both accuracy and efficiency.
	- [Code](https://github.com/XieResearchGroup/Physics-aware-Multiplex-GNN) : Last updated in May 2024, 1 months ago
	- [Publication](https://doi.org/10.25740/bn398fc4306) : Citations: 0.0
- **PIGNet**: physics-informed deep learning model toward generalized drug–target interaction predictions
	- [Code](https://github.com/ACE-KAIST/PIGNet) : Last updated in February 2022, 28 months ago
	- [Publication](https://doi.org/10.1039/D1SC06946B) : Citations: 70.0
- **PIGNet2**: A versatile deep learning-based model for protein-ligand interaction prediction.
	- [Code](https://github.com/ACE-KAIST/PIGNet2) : Last updated in March 2024, 3 months ago
	- [Publication](https://doi.org/10.1039/D3DD00149K) : Citations: 3.0
- **PLANET**: A graph neural network model for predicting protein-ligand binding affinity and virtual screening.
	- [Code](https://github.com/ComputArtCMCG/PLANET) : Last updated in December 2023, 5 months ago
	- [Publication](https://doi.org/10.1021/acs.jcim.3c00253) : Citations: 13.0
- **PLAPT**: A state-of-the-art protein-ligand binding affinity predictor leveraging transfer learning from pre-trained transformers to predict binding affinities with high accuracy.
	- [Code](https://github.com/trrt-good/WELP-PLAPT/tree/main) : Last updated in February 2024, 3 months ago
	- [Publication](https://doi.org/10.1101/2024.02.08.575577) : Citations: 0.0
- **ProSmith**: A multimodal transformer network for protein-small molecule interactions.
	- [Code](https://github.com/AlexanderKroll/ProSmith) : Last updated in April 2024, 1 months ago
	- [Publication](https://doi.org/10.5281/zenodo.10986299).) : Citations: 0.0
- **ProtMD**: Pre-training Protein Geometric Models via Molecular Dynamics Trajectories
	- [Code](https://github.com/smiles724/ProtMD) : Last updated in December 2022, 17 months ago
	- [Publication](https://doi.org/10.1002/advs.202203796) : Citations: 11.0
- **QDL-DTA**: A hybrid quantum-classical deep learning algorithm for protein-ligand binding affinity prediction.
	- [Code](https://github.com/LinaDongXMU/QDL-DTA) : Last updated in July 2023, 10 months ago
	- [Publication](https://doi.org/10.1002/qute.202300107.) : Citations: 0.0
- **RoseNet**: Predicts absolute binding affinity using molecular mechanics energies.
	- [Code](https://github.com/DS3Lab/RosENet/tree/master) : Last updated in March 2021, 39 months ago
	- [Publication](https://doi.org/10.5281/zenodo.4625486) : Citations: 0.0
- **RTMScore**: A novel scoring function based on residue-atom distance likelihood potential and graph transformer for predicting protein-ligand interactions efficiently.
	- [Code](https://github.com/sc8668/RTMScore) : Last updated in April 2023, 14 months ago
	- [Publication](https://doi.org/10.1021/acs.jmedchem.2c00991) : Citations: 55.0
- **SadNet**: 
	- [Code](https://github.com/wardhong/SadNet) : Last updated in March 2024, 3 months ago
	- [Publication](https://doi.org/10.1039/D3CP05664C) : Citations: 0.0
- **SCORCH**: Houses deployable code for the SCORCH scoring function and docking pipeline.
	- [Code](https://github.com/SMVDGroup/SCORCH) : Last updated in July 2022, 22 months ago
	- [Publication](https://doi.org/10.1111/cbdd.13764) : Citations: 14.0
- **SIEVE-Score**: A virtual screening method based on random forest using interaction energy-based scoring.
	- [Code](https://github.com/sekijima-lab/SIEVE-Score) : Last updated in September 2023, 8 months ago
	- [Publication](https://doi.org/10.1021/acs.jcim.8b00673) : Citations: 90.0
- **SMPLIP-Score**: Substructural Molecular and Protein–Ligand Interaction Pattern Score, a direct interpretable predictor of absolute binding affinity.
	- [Code](https://github.com/college-of-pharmacy-gachon-university/SMPLIP-Score) : Last updated in June 2021, 36 months ago
	- [Publication](https://doi.org/10.1186/s13321-021-00507-1) : Citations: 27.0
- **SSnet**: Secondary Structure based End-to-End Learning model for Protein-Ligand Interaction Prediction
	- [Code](https://github.com/ekraka/SSnet) : Last updated in November 2021, 31 months ago
	- [Publication](https://doi.org/10.3390/ijms22031392) : Citations: 29.0
- **StackCPA**: A stacking model for compound-protein binding affinity prediction based on pocket multi-scale features (scoring - 2023) (standalone).
	- [Code](https://github.com/CSUBioGroup/StackCPA) : Last updated in April 2023, 14 months ago
	- [Publication](https://doi.org/10.1016/j.compbiomed.2023.107131) : Citations: 2.0
- **TankBind**: Trigonometry-Aware Neural NetworKs for Drug-Protein Binding Structure Prediction
	- [Code](https://github.com/luwei0917/TankBind) : Last updated in November 2023, 7 months ago
	- [Publication](https://doi.org/10.1101/2022.06.06.495043) : Citations: 44.0
- **TB-IEC-Score**: Meta-modeling of ligand-protein binding affinity.
	- [Code](https://github.com/schrojunzhang/TB-IEC-Score) : Last updated in June 2023, 12 months ago
	- [Publication](https://doi.org/10.1186/s13321-023-00731-x) : Citations: 5.0
- **TrustAffinity**: A sequence-based deep learning framework addressing challenges in protein-ligand binding affinity prediction including out-of-distribution generalizations, uncertainty quantification, and scalability.
	- [Publication](https://doi.org/10.1101/2024.01.05.574359) : Citations: 0.0
	- [Link](https://www.biorxiv.org/content/10.1101/2024.01.05.574359v1.full)
- **XLPFE**: a Simple and Effective Machine Learning Scoring Function for Protein-ligand Scoring and Ranking (standalone).
	- [Code](https://github.com/LinaDongXMU/XLPFE) : Last updated in June 2023, 11 months ago
	- [Publication](https://doi.org/10.1021/acsomega.2c01723.) : Citations: 0.0
- **XLPFE**: A machine learning scoring function for protein-ligand scoring and ranking.
	- [Code](https://github.com/LinaDongXMU/XLPFE) : Last updated in June 2023, 11 months ago
	- [Publication](https://doi.org/10.1021/acsomega.2c01723.) : Citations: 0.0

## **Metalloproteins Specific**
- **MetalProGNet**: MetalProGNet is a structure-based deep graph model specifically designed for metalloprotein-ligand interaction predictions, developed based on the IGN framework.
	- [Code](https://github.com/zjujdj/MetalProGNet) : Last updated in November 2022, 19 months ago
	- [Publication](https://doi.org/10.1039%2Fd2sc06576b) : Citations: 6.0

## **Protein-ligand interaction fingerprints**
- **ECIFGraph**: Introduces a water network-augmented two-state model for protein-ligand binding affinity prediction, incorporating extended connectivity interaction features and graph transformer operators.
	- [Code](https://github.com/xiaoyangqu/ECIFGraph) : Last updated in June 2023, 12 months ago
	- [Publication](https://doi.org/10.1021/acs.jcim.3c00567) : Citations: 3.0

## **Quantum-mechanical**
- **SQM2.20**: Semiempirical quantum-mechanical scoring function yields DFT-quality protein–ligand binding affinity predictions in minutes
	- [Publication](https://doi.org/10.1038/s41467-024-45431-8) : Citations: 2.0
	- [Link](https://chemrxiv.org/engage/chemrxiv/article-details/64afd3aeae3d1a7b0d9ac7c0)

## **RNA-specific**
- **RLAffinity**: 
	- [Code](https://github.com/SaisaiSun/RLaffinity) : Last updated in May 2024, 0 months ago
	- [Publication](https://doi.org/10.1093/bioinformatics/btae155) : Citations: 0.0

## **Relative Binding Affinity**
- **PBCNet**: Webserver for relative binding affinity calculation.
	- [Webserver](https://pbcnet.alphama.com.cn/index)

## **SF Optimisation**
- **SANDRES**: Statistical Analysis of Docking Results and Scoring functions
	- [Link](https://azevedolab.net/sandres.php)
- **SFSXplorer**: Computational tool to explore the scoring function space (standalone).
	- [Code](https://github.com/azevedolab/SFSXplorer) : Last updated in January 2024, 4 months ago
	- [Link](https://azevedolab.net/sfsxplorer.php)
